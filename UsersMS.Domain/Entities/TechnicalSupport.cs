...
{
    public class TechnicalSupport : AggregateRoot<UserId>
    {
        ...
        public TechnicalSupport(
            ...
            : base(id)
        {
            Name = name ?? throw new ArgumentNullException(nameof(name), "El técnico debe tener un nombre.");
            Email = email ?? throw new ArgumentNullException(nameof(email), "El técnico debe tener un email.");
            Phone = phone ?? throw new ArgumentNullException(nameof(phone), "El técnico debe tener un teléfono.");
            Cedula = cedula ?? throw new ArgumentNullException(nameof(cedula), "El técnico debe tener una cédula.");
            IsVerified = isVerified;
            Token = token;

            ValidateState();
            AddDomainEvent(new RecordTechnicalSupportDataEvent(id, name, email, phone, cedula, isVerified));
        }

        protected override void ValidateState()
        {
            if (Name == null) throw new InvalidUserNameException("El técnico debe tener un nombre.");
            if (Email == null) throw new InvalidUserEmailException("El técnico debe tener un email.");
            if (Phone == null) throw new InvalidUserPhoneException("El técnico debe tener un teléfono.");
            if (Cedula == null) throw new InvalidUserCedulaException("El técnico debe tener una cédula.");
        }
