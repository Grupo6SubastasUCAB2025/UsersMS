...
namespace UsersMS.Application.Decorators
{
    public class RecordUserDataSecurityDecorator<TRequest, TResponse> : IService<TRequest, TResponse>
    {
        ...
        public async Task<TResponse> Execute(TRequest request)
        {
            ...
            var dynamicRequest = request as dynamic;

            if (dynamicRequest == null)
                throw new UnauthorizedException("Invalid request type.");

            if (email != dynamicRequest.UserEmail ||
                role != dynamicRequest.Role ||
                userId != dynamicRequest.UserId.ToString())
            {
